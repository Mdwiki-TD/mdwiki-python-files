...
casesin_to_urls = [
    ids_to_urls.get(str(ca_id)) for ca_id in cases_in_ids
    if ids_to_urls.get(str(ca_id))
]
...
print(
    f"Length of jsons.cases_in: {len(cases_in_ids)}, Length of casesin_to_urls: {len(casesin_to_urls)}"
)
