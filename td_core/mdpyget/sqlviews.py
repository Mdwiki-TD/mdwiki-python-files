...
def update_2023(lang, table):
    ...
    for target, tab in table.items():
        ...
        if "nosql" not in sys.argv:
            ...
            # ---
            printe.output(f"<<yellow>>sqlviews.py mdwiki_sql result:{str(qu)}")
...
def get_views_sql(lang_o):
    ...
    for tab in dad:
        ...
        if lang not in already_in_sql:
            already_in_sql[lang] = {}
        # ---
        already_in_sql[lang][target] = {"all": countall, "2021": count2021, "2022": count2022, "2023": count2023, "2024": count2024, "2025": count2025, "2026": count2026}
