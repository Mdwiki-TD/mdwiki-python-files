...
> **Disclaimer:** This is *static* analysis. Files flagged as unused are candidates only — verify before deletion. Dynamic imports, CLI tools invoked outside `pwb.py`, cron jobs, shell scripts, and pytest-driven tests can keep code "alive" in ways an import trace will not capture.
...
> *Note:* this folder has no `__init__.py` and contains a non-Python `list.wiki` data file as well.
...
| Path | Reason |
|---|---|
| `src/md_core_helps/apis/wd_bots/wd_post_new.py` | Not imported (its sibling `wd_rest_new.py` and `wikidataapi_post.py` *are* imported). |
...
### `src/sqlalchemy_sql/` (entire package)

No active entry uses any sqlalchemy_sql module — every active script reaches DB code via `md_core_helps/mdapi_sql/*` instead.

| Path | Reason |
|---|---|
| `src/sqlalchemy_sql/__init__.py` | No active entry. |
| `src/sqlalchemy_sql/config.py` | — |
| `src/sqlalchemy_sql/shared/__init__.py` | — |
| `src/sqlalchemy_sql/shared/engine.py` | — |
| `src/sqlalchemy_sql/shared/services/__init__.py` | — |
| `src/sqlalchemy_sql/shared/services/allqid_service.py` | — |
| `src/sqlalchemy_sql/shared/services/assessment_service.py` | — |
| `src/sqlalchemy_sql/shared/services/category_service.py` | — |
| `src/sqlalchemy_sql/shared/services/enwiki_pageview_service.py` | — |
| `src/sqlalchemy_sql/shared/services/in_process_service.py` | — |
| `src/sqlalchemy_sql/shared/services/lang_service.py` | — |
| `src/sqlalchemy_sql/shared/services/language_setting_service.py` | — |
| `src/sqlalchemy_sql/shared/services/mdwiki_revid_service.py` | — |
| `src/sqlalchemy_sql/shared/services/page_service.py` | — |
| `src/sqlalchemy_sql/shared/services/pages_users_to_main_service.py` | — |
| `src/sqlalchemy_sql/shared/services/project_service.py` | — |
| `src/sqlalchemy_sql/shared/services/qid_service.py` | — |
| `src/sqlalchemy_sql/shared/services/refs_count_service.py` | — |
| `src/sqlalchemy_sql/shared/services/report_service.py` | — |
| `src/sqlalchemy_sql/shared/services/translate_type_service.py` | — |
| `src/sqlalchemy_sql/shared/services/user_page_service.py` | — |
| `src/sqlalchemy_sql/shared/services/user_service.py` | — |
| `src/sqlalchemy_sql/shared/services/views_new_service.py` | — |
| `src/sqlalchemy_sql/shared/services/word_service.py` | — |
| `src/sqlalchemy_sql/sqlalchemy_models/__init__.py` | — |
| `src/sqlalchemy_sql/sqlalchemy_models/all_articles.py` | — |
| `src/sqlalchemy_sql/sqlalchemy_models/dashboard.py` | — |
| `src/sqlalchemy_sql/sqlalchemy_models/metrics.py` | — |
| `src/sqlalchemy_sql/sqlalchemy_models/pages.py` | — |
| `src/sqlalchemy_sql/sqlalchemy_models/public.py` | — |
| `src/sqlalchemy_sql/sqlalchemy_models/publish.py` | — |
| `src/sqlalchemy_sql/sqlalchemy_models/qid.py` | — |
| `src/sqlalchemy_sql/sqlalchemy_models/setting.py` | — |
| `src/sqlalchemy_sql/sqlalchemy_models/users.py` | — |
| `src/sqlalchemy_sql/sqlalchemy_models/views.py` | — |
...
| `src/sqlalchemy_sql/` (recursive) | 35 files — entire package. |
...
These files are *not* reachable through static AST tracing from the 28 active entry points, but **should not be deleted without human review** for the reasons listed.
...
The task statement says `pwb.py` is the *only* execution path, but the names below suggest CLI scripts that might be invoked directly. **Confirm before deleting.**
...
| `src/newupdater/**` and `src/sqlalchemy_sql/**` | Entire packages with non-trivial code — possibly under construction or invoked from another orchestrator. |
| `src/wprefs/**` | Has accompanying tests under `tests/src/wprefs/`; may be a library that is *only* exercised via tests today. |
...
2. **`from pkg.mod import Name` is handled both ways** — the trace marks `pkg/mod.py` as used *and* tries `pkg/mod/Name.py` (in case `Name` is a submodule).
...
4. **External libraries** (`mwclient`, `mwparserfromhell`, `pywikibot`, `requests`, `SQLAlchemy`, etc.) are excluded from the trace by checking against the eight known local top-level packages: `copy_to_en`, `md_core`, `md_core_helps`, `mdwiki_api`, `newupdater`, `sqlalchemy_sql`, `td_core`, `wprefs`.
