...
def _mdwiki_sql(query, values=None) -> list[dict[str, Any]]:
    if not query:
        logger.info("query == ''")
        return []

    return tf_sql_connect_dict(
        query,
        values=values,
        many=False,
    )
...
def get_all_qids() -> dict[str, str]:
    # ---
    # xxxx iiii oooo gggg
    # ---
    sq = _mdwiki_sql(f"select DISTINCT title, qid from {TABLE_NAME};")
