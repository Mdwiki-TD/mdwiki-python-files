...
def _create_default_regexes() -> None:
    ...
    _regex_cache.update({
        ...
        "interwiki": (
            r"\[\[:?(%s)\s?:[^\]]*\]\]\s*",
            lambda site: "|".join(
                ignore_case(i) for i in site.validLanguageLinks() + list(
        ...
        "pagelist":
        re.compile(r"<{}[\s\S]*?/>".format(ignore_case("pagelist"))),
        ...
        if exc in _regex_cache:
            if isinstance(_regex_cache[exc], tuple):
                if not site and exc in (
                    ...
                    raise ValueError(
                        f"Site cannot be None for the '{exc}' regex")

                if (exc, site) not in _regex_cache:
                    re_text, re_var = _regex_cache[exc]
                    _regex_cache[(exc,
                                  site)] = re.compile(re_text % re_var(site),
                                                      re.VERBOSE)
...
def replaceExcept(
    ...
    while not count or replaced < count:
        ...
        for dontTouchR in dontTouchRegexes:
            ...
            if excMatch and (nextExceptionMatch is None
